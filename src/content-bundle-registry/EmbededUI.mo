...
module {
	...
	let CLASS_CSS =  "<style type=\"text/css\"> .grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; } .details { max-width:900px; } .logo { max-height:200px; margin: 1em auto; display:block; } .logo_details { max-height:350px; margin: 1em auto; display:block; } .cell { min-height: 100px; border: 1px solid gray; border-radius: 8px; padding: 8px 16px; position: relative; } .cell_details { min-height: 250px; border: 1px solid gray; border-radius: 8px; padding: 8px 8px; position: relative; } .tag { color:#1414A8; margin: 0 4px; border: 1px solid #2C44DA; border-radius: 8px; padding: 4px 16px; background-color:#B6E3FF;} .right {float:right;}</style>";

    public func render_root_header (tag:?Text, total:Nat) : Text {
        var out_html = "<html><head>"#GLOBAL_CSS#CLASS_CSS# "</head><body>" # "<h2>&#128464; Content packages overview &#9757; </h2><span><i>Total packages</i> : <span style=\"padding: 0 20 0 5; font-weight:bold;\">"# Nat.toText(total) # "</span><hr/>";
		switch (tag) {
    ...
    public func page_response(canister_id:Text, network:CommonTypes.Network, key : Text, target_package:?Types.BundlePackage) : ICS2Http.Response {
		switch (target_package) {
            ...
            case (? v)  {
				...
				var directory_html = "<html><head>"#GLOBAL_CSS#CLASS_CSS#"</head><body>" # "<h2><span><a style=\"margin: 0 5px;\" href=\"" # root_url # "\" >"#Utils.ROOT#"</a></span>  &#128464; Package : "#v.name#" </h2><hr/><div class=\"details\">";
