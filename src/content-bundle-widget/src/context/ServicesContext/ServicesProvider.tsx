...
const ServicesProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const widgetService = useMemo(() => {
    return identity ? new WidgetService(identity) : new WidgetService()
  }, [identity])

  const initBundlePackageService = useCallback((packageId: string) => new BundlePackageService(packageId), [])

  return (
    <ServicesContext.Provider value={{ widgetService, initBundlePackageService }}>{children}</ServicesContext.Provider>
  )
}
