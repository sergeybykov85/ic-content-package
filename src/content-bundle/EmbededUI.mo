...
module {
	...
	let CLASS_CSS =  "<style type=\"text/css\"> .grid2 { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; } .grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; } .details { max-width:1100px; } .logo { max-height:200px; margin: 1em auto; display:block; } .logo_details { max-height:30px; margin: 1em auto; display:block; } .cell { min-height: 100px; border: 1px solid gray; border-radius: 8px; padding: 8px 16px; position: relative; } .cell_details { min-height: 250px; border: 1px solid gray; border-radius: 8px; padding: 8px 8px; position: relative; } .tag { color:#1414A8; margin: 0 4px; border: 1px solid #2C44DA; border-radius: 8px; padding: 4px 10px; background-color:#B6E3FF;} .classification { color:#f7f2f2; margin: 0 4px; border: 2px solid #0a0a0a; border-radius: 15px; padding: 6px 10px; background-color:#424040;} .right {float:right;}</style>";
	...
	public func render_bundle_overview (canister_id: Text, network:CommonTypes.Network, id:Text, r:Types.Bundle) : Text {
		...
		if (List.size(r.index.tags) > 0) {
			let tags_fmt = Text.join("", List.toIter(List.map(r.index.tags, func (t : Text):Text {"<span class=\"tag\">"#t#"</span>";})));
	...
	public func render_bundle_details (canister_id: Text, network:CommonTypes.Network, id:Text, r:Types.Bundle) : Text {
		...
		resource_html :=resource_html # "<div>&#128464; <a style=\"font-weight:bold; color:#2C44DA;\" href=\"" # url # "\" target = \"_self\">"# r.name # "</a><span class=\"classification right\">"# r.index.classification #"</span></div>";
